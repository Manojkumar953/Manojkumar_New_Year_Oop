...
public class SweetsGenerator implements Generator<Sweets>, Iterable<Sweets> {
    ...
    private Sweets[] instances = {new DairyMilk(), new MilkChocolate(),
            new Kova(), new Jalebi()   };
    public Sweets next() {
    	try {
    		Sweets current = (Sweets)instances[rand.nextInt(instances.length)].clone();
	...
	class SweetIterator implements Iterator<Sweets> {
		int count = size;
